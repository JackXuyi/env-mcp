...
describe('handleCallToolRequest', () => {
  it('应该返回系统信息', async () => {
    const result = await handleCallToolRequest({ params: { name: 'getSystemInfo' } });
  ...
  it('应该返回 CPU 使用率', async () => {
    const result = await handleCallToolRequest({ params: { name: 'getCpuUsage' } });
    const cpuUsage = JSON.parse(result.content[0].text);
    expect(cpuUsage).toHaveProperty('cpuUsage');
  });

  it('应该返回硬盘使用率', async () => {
    const result = await handleCallToolRequest({ params: { name: 'getDiskUsage' } });
    expect(result.content[0].text).toBeDefined();
  });

  it('应该返回终端类型', async () => {
    const result = await handleCallToolRequest({ params: { name: 'getTerminalTypes' } });
    const terminalTypes = JSON.parse(result.content[0].text);
    expect(terminalTypes).toHaveProperty('terminalTypes');
  });

  it('应该抛出未知工具错误', async () => {
    await expect(handleCallToolRequest({ params: { name: 'unknownTool' } }))
      .rejects
      .toThrow('未知的工具');
  });
}); 